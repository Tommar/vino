        ...
        if (JRequest::get('post') && JRequest::getVar('btqc' . $module->id)) {
			...
			if($this->_result){
				if(!JSession::checkToken()){
					$this->_result = false;
                    $this->_errorMessages[] = JText::_('JINVALID_TOKEN');
				}
			}
            if ($this->_result) {
                //send mail
			 ...
			 * @since 2.1.4
			 */
			$this->_jsonResponse['bttoken'] = JSession::getFormToken(true);
        ...
        if ($params->get('subject')) {
			...
			if(!empty($matches)){
				foreach($matches as $m){
					$alias = trim($m, '{}');
					if(key_exists('btqc_f_' . $alias, $data)){
						if (is_array($data['btqc_f_' . $alias])){
							$data['btqc_f_' . $alias] = implode(', ', $data['btqc_f_' . $alias]);
						}
						$subject = str_replace($m, $data['btqc_f_' . $alias], $subject);
					}
				}
			}
        } else {
