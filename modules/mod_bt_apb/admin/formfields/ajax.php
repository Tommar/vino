...
class JFormFieldAjax extends JFormField {
    ...
    protected function getInput() {
        ...
        $this->saveDir = JPATH_ROOT .'/modules/mod_bt_apb/images';
         ...
         */ 
            ...
            if (!$this->isWritable()) {
                ...
                if (!$isAdmin) {
                     ...
                     */
                    //nếu là uploadify
                    if (JRequest::getString('action') == 'upload' && !empty($_FILES)) {
                        ...
                        if (JRequest::getString('action') == 'get') {
                            ...
                            if (count($photos) > 0) {
                                ...
                                if (!isset($this->result['message']) || $this->result['message'] == '')
                                    $this->result["message"] = JText::_('ERROR_NO_IMAGE');
                            }
                            return json_encode($this->result);
	...
	function getFlickrPhotosets(){
		...
		if (count($arrOptions) > 0) {
			...
			$objOption = new stdClass();
			$objOption->value = 0;
			$objOption->text = JText::_('FLICKR_ALL_PHOTOSETS');
			$arrOptions[] = $objOption;
			$arrOptions = array_reverse($arrOptions);
		}
		return $arrOptions;
	...
	function getPicasaAlbums(){
		...
		if (isset($sxml) && $sxml) {
			...
			if (count($arrOptions) > 0) {
				...
				$objOption = new stdClass();
				$objOption->value = 0;
				$objOption->text = JText::_('PICASA_ALL_ALBUMS');
				$arrOptions[] = $objOption;
				$arrOptions = array_reverse($arrOptions);
			}
		} else {
	...
	function getImagesFromJFolder(& $photos){
		...
		if (is_dir($jFolderPath)) {
			...
			$this->result['message'] = JText::_('ERROR_DIRECTORY');
			$validated = false;
		}
	}
