...
class BtApbHelper {
    ...
    public static function fetchHead($params){
		...
		if($loadJquery)
		{
			$document->addScript(JURI::root().'modules/mod_bt_apb/tmpl/js/jquery.min.js');
		}
		
		$document->addScript(JURI::root().'modules/mod_bt_apb/assets/js/jquery.easing.1.3.js');
		
		if($params->get('parallax_type', 'dynamic') == 'dynamic'){
			$document->addScript(JURI::root().'modules/mod_bt_apb/assets/js/jquery.parallax-1.1.3.js');
		}
		if(file_exists(JPATH_BASE.'/templates/'.$template.'/html/mod_bt_apb/css/style.css'))
		{
			$document->addStyleSheet(  JURI::root().'templates/'.$template.'/html/mod_bt_apb/css/style.css');
		}
		else{
			$document->addStyleSheet(JURI::root().'modules/mod_bt_apb/tmpl/css/style.css');
		}
		$document->addScript(JURI::root().'modules/mod_bt_apb/assets/js/jquery.transform2d.js');
		if(file_exists(JPATH_BASE.'/templates/'.$template.'/html/mod_bt_apb/js/jquery.apb-1.0.0.js')){
			$document->addScript(JURI::root().'templates/'.$template.'/html/mod_bt_apb/js/jquery.apb-1.0.0.js');
		}else{
			$document->addScript(JURI::root().'modules/mod_bt_apb/assets/js/jquery.apb-1.0.0.js');	
		}
        $document->addScript(JURI::root().'modules/mod_bt_apb/assets/js/script.js');    
	}
