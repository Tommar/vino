			...
			<div class="acymailing_form">
					...
					<?php foreach($fieldsToDisplay as $oneField){
						if($oneField == 'name' AND empty($extraFields[$oneField])){
							if($displayOutside) echo '<div><label for="user_name_'.$formName.'" class="acy_requiredField">'.$nameCaption.'</label></div>'; ?>
							<div class="acyfield_<?php echo $oneField. (!$displayOutside? ' acy_requiredField':''); ?>">
							...
							if($displayOutside) echo '<div><label for="user_email_'.$formName.'" class="acy_requiredField">'.$emailCaption.'</label></div>'; ?>
							<div class="acyfield_<?php echo $oneField. (!$displayOutside? ' acy_requiredField':''); ?>">
							...
							echo '>'.JText::_('RECEIVE').JHTML::_('select.booleanlist', "user[html]" ,'title="'.JText::_('RECEIVE').'"',isset($identifiedUser->html) ? $identifiedUser->html : 1,JText::_('HTML'),JText::_('JOOMEXT_TEXT'),'user_html_'.$formName).'</div>';
						}elseif(!empty($extraFields[$oneField])){
							if($displayOutside){
								if(!empty($extraFields[$oneField]->required)) $requireClass = 'class="acy_requiredField"';
								else $requireClass = "";
								echo '<div><label '.((strpos($extraFields[$oneField]->type,'text') !== false) ? 'for="user_'.$oneField.'_'.$formName.'"' : '' ).' '. $requireClass .'>'.$fieldsClass->trans($extraFields[$oneField]->fieldname).'</label></div>';
							}
							$sizestyle = '';
							...
							?>
							<div class="acyfield_<?php echo $oneField .' '. $requireClass; ?>">
